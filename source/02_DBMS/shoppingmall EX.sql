...
-- 주문번호 가져오기
SELECT TO_CHAR(SYSDATE, 'RRMMDD')||TRIM(TO_CHAR(ORDERS_SEQ.NEXTVAL, '000')) 
    FROM DUAL;
SELECT TO_CHAR(SYSDATE, 'RRMMDD')||SUBSTR(TO_CHAR(ORDERS_SEQ.NEXTVAL, '000'),2,3) 
    FROM DUAL;
DROP SEQUENCE ORDERS_SEQ;
CREATE SEQUENCE ORDERS_SEQ MAXVALUE 999 NOCACHE;
...
SELECT * FROM ORDERS WHERE ONO=230102004;
