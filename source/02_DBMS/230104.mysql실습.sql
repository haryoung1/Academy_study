...
-- MAJOR (mCODE;pk학과번호, mName 학과이름, mOffice 학과사무실)
-- STUDENT (sNO학번, sName이름, mCode 학과번호)
...
create table student (
    ...
    mCode int,
    primary key (sNo),
    foreign key (mCode) references major (mCode) -- 외래키 제약조건은 아래만 가능
);
...
insert into division values (40, 'cs', '031-444-4321', '안양');
select * from division;
commit;
...
select pno, pname, pay, pay*12+ ifnull (bonus,0) 
	from personal;
select pno, pname bonus, if(bonus is null, 0, bonus) 
	from personal;
