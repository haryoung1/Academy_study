...
SELECT * FROM USER_CONSTRAINTS 
    WHERE TABLE_NAME='DEPT1'; -- 테이블 제약조건 확인 ?
...
CREATE TABLE DEPT1 (
    DEPTNO NUMBER(2),  
    DNAME VARCHAR2(14) NOT NULL, -- 제약조건 NOT NULL은 오른쪽에만 사용 가능하다
    LOC   VARCHAR2(13) NOT NULL,
...
CREATE TABLE EMP1 (
    ...
    PRIMARY KEY (EMPNO),
    CHECK (SAL>0),
    FOREIGN KEY (DEPTNO) REFERENCES DEPT1 (DEPTNO) -- MY SQL 문법
);         
DESC EMP1;    
SELECT * FROM DEPT1;

-- 데이터를 넣을때는 부모테이블부터 넣어야함.
INSERT INTO DEPT1 SELECT * FROM DEPT;
INSERT INTO DEPT1 (DEPTNO, LOC, DNAME) VALUES (50, 'SEOUL', 'IT'); 
...
INSERT INTO EMP1 
    VALUES (1003, 'KIM', 'MANAGER', NULL, NULL, NULL, NULL, 90); --에러 페런츠 키 낫파운드
...
DROP TABLE BOOKCATEGORY CASCADE CONSTRAINTS; -- 참조하는 테이블과 상관없이 전부 삭제 (비추)
...
INSERT INTO BOOK VALUES (300, '300A01', '그랩장인', '라이엇출판',EXTRACT(YEAR FROM SYSDATE)); 
 
SELECT BOOKNO 책번호, BOOKNAME 책이름, PUBLISHER 출판사, PUBYEAR 출판년도,
       CATEGORYNAME 책분류, OFFICE_LOC 사무실
    FROM BOOK B, BOOKCATEGORY Y
    WHERE B.CATEGORYCODE = Y.CATEGORYCODE;
...
DELETE FROM BOOKCATEGORY 
    WHERE CATEGORYCODE = 100; -- CHILD에서 참조하고 있는 데이터
 
DELETE FROM BOOK 
    WHERE CATEGORYCODE = 100;
 
DELETE FROM BOOKCATEGORY
    WHERE CATEGORYCODE = 100; -- 참고하는 데이터가 없으면 삭제 가능
 
DELETE FROM BOOKCATEGORY
    WHERE CATEGORYCODE IN (200, 300);
...
-- ★  DCL (사용자 계정생성, 사용자 권한부여, 권한박탈, 사용자 계정삭제)
CREATE USER scott2 IDENTIFIED BY tiger; -- scott2 계정생성
...
-- 권한 박탈 (DEPT 테이블, EMP 테이블 권한) : 접속해제 후 할 것을 추천
REVOKE ALL ON EMP FROM SCOTT2;
REVOKE SELECT ON DEPT FROM SCOTT2;
    
-- 사용자 계정 삭제 (접속중인 계정은 삭제불가)
