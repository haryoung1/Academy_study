...
SELECT * 
    FROM EMP, 
    DEPT WHERE ENAME='SCOTT'; -- 1(EMP테이블의 갯수)*4(DEPT테이블갯수)

-- ★ 1. EQUI JOIN(공통필드인 DEPTNO 값이 일치되는 조건만 JOIN)
SELECT * 
    FROM EMP, DEPT 
...
SELECT ENAME, D.DEPTNO, LOC
    ...
    -- ex. LOC이 CHICAGO 인 사람의 이름, 업무, 급여, 부서명, 근무지 출력
SELECT ENAME, JOB, SAL, DNAME, LOC
    FROM EMP E, DEPT D
...
SELECT ENAME, JOB, LOC
    ...
    -- ex. JOB이 SALESMAN 이거나 MANAGER인 사원의 이름, 급여, 상여,연봉((SAL+COMM)*12)
    -- 부서명,근무지를 출력 (연봉이 큰 순으로 정렬)
SELECT ENAME, SAL, COMM, JOB, (SAL+NVL(COMM, 0))*12 ANNUALSAL, DNAME, LOC
    FROM EMP E, DEPT D
    WHERE E.DEPTNO = D.DEPTNO AND JOB IN ('SALESMAN', 'MANAGER')
    ORDER BY ANNUALSAL DESC;

    -- ex. COMM이 NULL이고 SAL이 1200이상인 사원의 이름, 급여, 입사일, 부서번호, 부서명
    -- (부서명 순, 부서명이 같을 시 급여 큰 순 정렬)
SELECT ENAME, SAL, HIREDATE, D.DEPTNO, DNAME
    ...
    -- 탄탄 EX1. NEW YORK 에서 근무하는 사원의 이름과 급여를 출력하시오
SELECT ENAME, SAL
    FROM EMP E, DEPT D
    WHERE E.DEPTNO = D.DEPTNO AND LOC = 'NEW YORK';
    
    -- 탄탄 EX2. ACCOUNTING 부서 소속 사원의 이름과 입사일을 출력하시오
SELECT ENAME, HIREDATE
    FROM EMP E, DEPT D
    WHERE E.DEPTNO = D.DEPTNO AND DNAME = 'ACCOUNTING';

    -- 탄탄 EX3.직급이 MANAGER인 사원의 이름, 부서명을 출력하시오
SELECT ENAME, DNAME
    FROM EMP E, DEPT D
    WHERE E.DEPTNO = D.DEPTNO AND JOB = 'MANAGER';

    -- 탄탄 EX4. Comm이 null이 아닌 사원의 이름, 급여, 부서코드, 근무지를 출력하시오.
...
SELECT *
    FROM EMP, SALGRADE
    WHERE ENAME = 'SCOTT'; -- CROSS JOIN (의미 없음)
...
SELECT *
    ...
    -- ex. 모든사원의 사번, 이름, 직책, 상사사번, 급여, 급여등급 (1등급, 2등급) 출력
...
SELECT ENAME, SAL, HIREDATE, GRADE
    ...
    -- 탄탄 ex3 이름, 급여, 입사일, 급여등급, 부서명, 근무지
SELECT ENAME, SAL, HIREDATE, GRADE, DNAME, LOC
    FROM EMP E, DEPT D, SALGRADE
    WHERE E.DEPTNO = D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL;

    -- 탄탄 ex4 이름, 급여, 등급, 부서코드, 근무지. 단 comm 이 null아닌 경우
 SELECT ENAME, SAL, GRADE, D.DEPTNO, LOC
    FROM EMP E, DEPT D, SALGRADE
    WHERE E.DEPTNO = D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL AND COMM IS NULL;
    
    -- 탄탄 ex5 이름, 급여, 급여등급, 연봉, 부서명, 부서별 출력, 부서가 같으면 연봉순.연봉=(sal+comm)*12 comm이 null이면 0
SELECT ENAME, SAL, GRADE, (SAL+NVL(COMM,0))*12 ANNUALSAL, DNAME
    FROM EMP E, DEPT D, SALGRADE
    WHERE E.DEPTNO = D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL
    ORDER BY DNAME, ANNUALSAL;
    
    -- 탄탄 ex6 이름, 업무, 급여, 등급, 부서코드, 부서명 출력. 급여가 1000~3000사이. 
    -- 정렬조건 : 부서별, 부서같으면 업무별, 업무같으면 급여 큰순
SELECT ENAME, JOB, SAL, GRADE, D.DEPTNO, DNAME
    FROM EMP E, DEPT D, SALGRADE
    WHERE SAL BETWEEN LOSAL AND HISAL AND SAL BETWEEN 1000 AND 3000
    ORDER BY DNAME, JOB, SAL DESC;

    -- 탄탄 ex7 이름, 급여, 등급, 입사일, 근무지. 81년에 입사한 사람. 등급 큰순
SELECT ENAME, SAL, GRADE, HIREDATE, LOC
    FROM EMP,  DEPT,  SALGRADE
    WHERE SAL BETWEEN LOSAL AND HISAL 
    AND TO_CHAR(HIREDATE, 'RR/MM/DD') LIKE '81%'
    ORDER BY GRADE DESC;
...
SELECT W.EMPNO, W.ENAME, W.MGR, M.ENAME
    FROM EMP W, EMP M
    WHERE W.MGR = M.EMPNO; -- 13행 (KING의 MRG이 NULL이라서 EMPNO는 NULL 불가)
...
SELECT W.ENAME ||'의 상사는 '|| M.ENAME ||'다' MESSAGE
    ...
    -- 탄탄 ex1. 매니저가 KING인 사원들의 이름과 직급을 출력하시오.
SELECT W.ENAME, W.JOB 
    FROM EMP W, EMP M
    WHERE W.MGR = M.EMPNO AND M.ENAME = 'KING';

	-- 탄탄 ex2. SCOTT과 동일한 부서번호에서 근무하는 사원의 이름을 출력하시오
SELECT E2.ENAME
    FROM EMP E1, EMP E2
    WHERE E1.ENAME='SCOTT' AND E1.DEPTNO=E2.DEPTNO AND E2.ENAME <> 'SCOTT';
    
    
    -- 탄탄 ex3.SCOTT과 동일한 근무지에서 근무하는 사원의 이름을 출력하시오(2번 심화)
    -- hint :
...
-- ★ 4. OUTER JOIN ; EUIE JOIN & SELF JOIN시  조인시 조건에 만족하지 않는 행까지 나타나게
...
SELECT W.ENMAE ||'의 상사는'|| NVL(M.ENAME, '없다')
    FROM EMP W, EMP M
    WHERE W.MGR = M.EMPNO(+);
...
--3. 이름, 급여, 업무, 직속상사명.
    --(상사가 없는 직원까지 전체 직원 다 출력. 상사가 없을 시 '없음'으로 출력)
SELECT W.ENAME, W.SAL, W.JOB, M.ENAME MANAGER
    FROM EMP W, EMP M
    WHERE W.MGR = M.EMPNO(+);
...
SELECT W.ENAME, W.SAL, W.DEPTNO, D.DNAME, D.LOC, M.ENAME MANAGER
    FROM EMP W, EMP M, DEPT D
...
SELECT W.ENAME, W.SAL, GRADE, DNAME,(W.SAL+NVL(NULL, 0))*12 ANNUALSAL, M.ENAME MANAGER
...
SELECT ENAME, COMM, DNAME , LOC 
    FROM EMP E, DEPT D
    WHERE D.DEPTNO = E.DEPTNO AND COMM > 0;

--4. EMP 테이블에서 이름 중 L자가 있는 사원에 대하여 이름,업무,부서명,위치를 출력
SELECT E.ENAME, E.JOB, D.DNAME, D.LOC
    FROM EMP E, DEPT D
    WHERE E.DEPTNO = E.DEPTNO AND ENAME LIKE '%L%';
...
SELECT E.EMPNO, E.ENAME, E.SAL, D.DNAME
    FROM EMP E, DEPT D, SALGRADE
    WHERE E.DEPTNO = D.DEPTNO
    AND E.SAL BETWEEN LOSAL AND HISAL AND E.SAL>2000
    ORDER BY E.SAL DESC;
...
SELECT E.EMPNO, E.ENAME, E.JOB, E.SAL, D.DNAME
    FROM EMP E, DEPT D, SALGRADE
    WHERE E.DEPTNO = D.DEPTNO 
    AND E.SAL BETWEEN LOSAL AND HISAL AND E.SAL>2500
    AND JOB = 'MANAGER'
    ORDER BY E.EMPNO;
