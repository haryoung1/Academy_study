...
public class Ex3_LifeCycle extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		// 서블릿 객체가 생성될 때 실행
		System.out.println("Ex3_lifeCycle 서블릿 객체가 생성 (메모리에서 load)될때 수행 - 2");
	}

	public void destroy() {
		// 서블릿 객체가 소멸될 때 실행
		System.out.println("Ex3_lifeCycle 서블릿 객체가 소멸(메모리에서 사라질때) 수행 -1");
	}

	@PreDestroy
	private void preDestroy() {
		// 서블릿 객체가 소멸 되기 바로 전 실행
		System.out.println("Ex3_lifeCycle 서블릿 객체가 소멸되기 바로 전 수행 -2");
	}
	/*
	 * protected void service(HttpServletRequest request, HttpServletResponse
	 * response) throws ServletException, IOException { // service를 override 하면
