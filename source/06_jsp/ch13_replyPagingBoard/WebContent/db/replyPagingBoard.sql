...
INSERT INTO BOARD (NUM, WRITER, SUBJECT, CONTENT, EMAIL, PW, REF, RE_STEP, RE_INDENT, IP) 
    VALUES(143, '강', '글1', '본문', NULL, '111', 143, 0,0, '168.125.1.1');

-- 143번의 1번째 답글 달기
    -- 답변글 추가 전단계 (엑셀의 ⓐ단계)
    UPDATE BOARD SET RE_STEP = RE_STEP + 1 
        WHERE REF = 143 AND RE_STEP > 0;
        
    -- 답변글 INSERT (REF = 143, RE_STEP = 1, RE_INDENT = 1)
    INSERT INTO BOARD (NUM, WRITER, SUBJECT, CONTENT, EMAIL, PW, REF, RE_STEP, RE_INDENT, IP)
        VALUES (144, '답변자', '글1-답1', '본문', 'R@R.COM', '111', 143, 1, 1, '127.0.0.1');
    
-- 143번의 2번째 답글 달기
    -- 답변글 추가 전단계 (엑셀의 ⓐ단계)
    UPDATE BOARD SET RE_STEP = RE_STEP + 1 
        WHERE REF = 143 AND RE_STEP > 0; -- 8. 답변글 추가 전 단계
        
    -- 답변글 INSERT (원글의 REF = 143, RE_STEP = 1, RE_INDENT = 1)
    SELECT MAX(NUM) FROM BOARD; -- 현재 글 최대값 확인
    INSERT INTO BOARD (NUM, WRITER, SUBJECT, CONTENT, EMAIL, PW, REF, RE_STEP, RE_INDENT, IP)
        VALUES (145, '답변자2', '글1-답2', '본문', 'KIM@L.COM', '111', 143, 1, 1, '121,1.1.1');
SELECT * FROM BOARD ORDER BY REF DESC, RE_STEP ASC;


-- 145번의 1번째 답글 달기
    -- 답변글 추가 전단계 (엑셀의 ⓐ단계 - 원글의 REF:143, 원글의 RE_STEP:1, 원글의 RE_INDENT:1)
SELECT MAX(NUM) FROM BOARD; -- 최고 글번호
SELECT * FROM BOARD WHERE REF=(SELECT REF FROM BOARD WHERE NUM=143);

    UPDATE BOARD SET RE_STEP = RE_STEP + 1 
        WHERE REF = 143 AND RE_STEP > 1; -- 8. 답변글 추가 전 단계 (DAO)
        
    INSERT INTO BOARD (NUM, WRITER, SUBJECT, CONTENT, EMAIL, PW, REF, RE_STEP, RE_INDENT, IP)
        VALUES ((SELECT NVL(MAX(NUM), 0) + 1 FROM BOARD), 
        '답변자3', '145글답', '본문', 'KIM@L.COM', '111', 143, 2, 2, '121,1.1.1'); -- 9. 답변글 추가
