...
<body>
	<%
		...
		if(dto==null){
			response.sendRedirect(conPath+"/board_Practice/list.jsp");
		}else{
	...
	%>
			<table>
				...
				<tr>
					<td colspan="2">
						<button onclick="location.href='<%=conPath%>/board_Practice/updateForm.jsp?num=<%=num%>'">수정</button>
						<button onclick="location.href='<%=conPath %>/board_Practice/deleteForm.jsp?num=<%=num %>'">삭제</button>
						<button>답변</button>
						<button onclick="location.href='<%=conPath%>/board_Practice/list.jsp'">목록</button>
					</td>
				</tr>
			</table>
	<%}%>
