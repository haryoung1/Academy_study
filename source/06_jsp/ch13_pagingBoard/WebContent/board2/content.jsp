...
<body>
	<%
		...
		if(dto == null) {
			response.sendRedirect(conPath+"/board2/list.jsp");
		}else {
	...
	<table>
		...
		<tr>
			<td colspan="2">
				<button onclick="location.href='<%=conPath %>/board2/updateForm.jsp?num=<%=num %>&pageNum=<%=pageNum %>'">
					수정
				</button>
				<button onclick="location.href='<%=conPath %>/board2/deleteForm.jsp?num=<%=num %>&pageNum=<%=pageNum %>'">
					삭제
				</button>
				<button>답변</button>
				<button onclick="location.href='<%=conPath%>/board2/list.jsp?pageNum=<%=pageNum%>'">
					목록
				</button>
		</tr>
	</table>
