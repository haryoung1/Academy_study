...
public class FileboardDao {
	...
	public ArrayList<FileboardDto> listBoard(int startRow, int endRow) {
		...
		ResultSet rs = null;
		String sql = "SELECT *" + "    FROM (SELECT ROWNUM RN, A.* " + "    FROM (SELECT F.*, CNAME, CEMAIL"
				+ "    FROM FILEBOARD F, CUSTOMER C  " + "    WHERE F.CID = C.CID"
				+ "    ORDER BY FREF DESC, FRE_STEP) A )" + "    WHERE RN BETWEEN ? AND ?";
		try {
			conn = getConnection();
