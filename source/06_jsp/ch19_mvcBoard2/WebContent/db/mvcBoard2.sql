-- TABLE & SEQUENCE DROP
DROP TABLE MVC_BOARD2;
DROP SEQUENCE MVC_BOARD_SEQ2;
-- TABLE & SEQUENCE CREATE
CREATE TABLE MVC_BOARD2 (
    bID NUMBER(6),
    bNAME VARCHAR2(50) NOT NULL,
    bTITLE VARCHAR2(100) NOT NULL,
    bCONTENT VARCHAR2(1000),
    bDATE DATE DEFAULT SYSDATE NOT NULL, -- 작성일
    bHIT     NUMBER(6) DEFAULT 0 NOT NULL,  -- 조회수
    bGROUP NUMBER(6) NOT NULL, -- 원글이면 BID와 같고, 답변글의 경우 원글의 BGROUP과 같다
    bSTEP NUMBER(3) NOT NULL, -- 같은 그룹내 출력 순서
    bINDENT NUMBER(3) NOT NULL, -- 들여쓰기 정도
    bIP VARCHAR2(20),
    PRIMARY KEY (BID)
);
CREATE SEQUENCE MVC_BOARD_SEQ2
    MAXVALUE 999999
    NOCACHE
    NOCYCLE;

-- DUMMY DATA
SELECT * FROM MVC_BOARD2;

INSERT INTO MVC_BOARD2 (BID, BNAME, BTITLE, BCONTENT, BGROUP, BSTEP, BINDENT, BIP)
 VALUES (MVC_BOARD_SEQ2.NEXTVAL, '홍','제목', NULL, MVC_BOARD_SEQ2.CURRVAL, 0,0,'192.1.1.1');

INSERT INTO MVC_BOARD2 (BID, BNAME, BTITLE, BCONTENT, BGROUP, BSTEP, BINDENT, BIP)
 VALUES (MVC_BOARD_SEQ2.NEXTVAL, '홍2','제목2', NULL, MVC_BOARD_SEQ2.CURRVAL, 0,0,'192.2.2.2');

-- DAO에 들어갈 QUERY
-- 1. 글목록(startRow ~ endRow까지)
SELECT * FROM MVC_BOARD2 ORDER BY BGROUP DESC, BSTEP;

SELECT * 
    FROM (SELECT ROWNUM RN, A.*
        FROM (SELECT * FROM MVC_BOARD2 ORDER BY BGROUP DESC, BSTEP) A)
    WHERE RN BETWEEN 1 AND 2;

-- 2. 전체 글 갯수
SELECT COUNT(*) FROM MVC_BOARD2;

-- 3. 원글 쓰기
INSERT INTO MVC_BOARD2 (BID, BNAME, BTITLE, BCONTENT, BGROUP, BSTEP, BINDENT, BIP)
 VALUES (MVC_BOARD_SEQ2.NEXTVAL, '홍','TITLE', NULL, MVC_BOARD_SEQ2.CURRVAL, 0,0,'192.1.1.1');

-- 4. bID로 조회수 1 올리기
UPDATE MVC_BOARD2 SET bHIT = bHIT + 1 WHERE bID=1;

-- 5. bID로 DTO가져오기 (글상세보기, 글수정VIEW, 답변글VIEW)
SELECT * FROM MVC_BOARD2 WHERE BID=1;

-- 6. 글 수정
UPDATE MVC_BOARD2
    SET BNAME = '홍수정',
        BTITLE = '제목바꿈',
        BCONTENT = '본문바꿈',
        BIP = '163.1.1.1'
    WHERE BID = 1;

-- 7. 글 삭제
DELETE FROM MVC_BOARD2 WHERE bID=1;
SELECT * FROM MVC_BOARD2;

-- 8. 답변글 저장전 작업(STEP ⓐ : 예시에서는 2번글의 답변)
UPDATE MVC_BOARD2 SET BSTEP = BSTEP + 1
  WHERE BGROUP = 2 AND BSTEP > 0;

-- 9. 답변글 쓰기 (예시는 2번의 답변)
SELECT * FROM MVC_BOARD2 WHERE bID=2;
INSERT INTO MVC_BOARD2 (BID, BNAME, BTITLE, BCONTENT, BGROUP, BSTEP, BINDENT, BIP)
  VALUES (MVC_BOARD_SEQ2.NEXTVAL, '진','T', NULL, 2, 1, 1, '126.1.1.1');
COMMIT;
SELECT * FROM MVC_BOARD2 ORDER BY BGROUP DESC, BSTEP;









