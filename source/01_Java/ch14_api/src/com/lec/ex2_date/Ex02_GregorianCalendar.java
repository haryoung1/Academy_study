...
//		Calendar now = Calendar.getInstance(); // 날짜 객체 가져오기
		GregorianCalendar now = new GregorianCalendar();
		System.out.println(now);
//		2022년 12월 14일 수요일 오전 09시48분
		int year 	 = now.get(Calendar.YEAR);
		int month 	 = now.get(Calendar.MONTH) + 1; 	// 0월부터 인식하기 때문에 +1을 해줘야 달이 맞음.
		...
		System.out.printf(" %d시 %d분 %d초 %d(%s %d시 %d분 %d초)\n", hour24, minute, second, milliSec, (ampm == 0 ? "오전" : "오후"),
				hour, minute, second);
		// %d (정수)
