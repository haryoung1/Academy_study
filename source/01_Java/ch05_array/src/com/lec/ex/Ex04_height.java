...
public class Ex04_height {
	public static void main(String[] args) {
		String[] arrName = {"영희", "철수", "길동", "영수", "말자"}; // 입력하는 사람들의 이름을 받을 공간
		int[] arrHeight = new int[arrName.length]; // 키를 입력 받을 공간
		Scanner scanner = new Scanner(System.in);
		int totHeight = 0;
		// 키 입력 받기 -> totHeight 메모리에 누적.
		...
		scanner.close();
		// 가장 큰 학생과 가장 작은 학생
		int maxIdx=0, maxHeight=0;    // 최장신을 구하기 위한 변수들
		int minIdx=0, minHeight=9999; // 최단신을 구하기 위한 변수들
		for(int idx=0 ; idx<arrName.length ; idx++) {
