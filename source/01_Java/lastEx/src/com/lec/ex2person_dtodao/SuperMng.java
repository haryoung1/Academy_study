...
public class SuperMng {
	public static void main(String[] args) {
		...
		do {
			System.out.print("1:회원가입, 2:폰뒷2자리(FULL)검색, 3:물품구매, 4:레벨별출력, 5:전체출력," + " 6:탈퇴, 그외:종료 ☞");
			fn = scanner.next();
			switch (fn) {
			case "1":
				...
				System.out.println(
						result == CustomerDao.SUCCESS ? cname + "님 회원가입 감사. 포인트 1000점 넣음" : cname + "님 회원가입 실패");
				break;
			case "2":
				System.out.print("검색할 전화번호 뒷4자리나 전화번호전체 : ");
				String searchTel = scanner.next();
				customers = dao.phoneGetCustomers(searchTel);
			...
			case "3":
				...
				break;
			case "4":
				System.out.print("검색하고자 하는 레벨" + dao.getlevelname() + " : ");
