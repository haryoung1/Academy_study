		...
		<div>
			...
			<form action="${conPath }/book.do">
				<input type="hidden" name="method" value="list"> <select
					name="schItem">
					<option value=""
						<c:if test="${param.schItem eq '' }">selected="selected"</c:if>>검색조건</option>
					<option value="all"
						<c:if test="${param.schItem eq 'all' }">selected="selected"</c:if>>책이름+저자</option>
					<option value="btitle"
						<c:if test="${param.schItem eq 'btitle' }">selected="selected"</c:if>>책이름</option>
					<option value="bwriter"
						<c:if test="${param.schItem eq 'bwriter' }">selected="selected"</c:if>>저자</option>
				</select> 
			...
			<tr>
				<c:forEach var="book" items="${bookList }">
					<td>${book.btitle }<br> 
						...
						${book.bwriter } 著 <br> 출판일 : ${book.brdate }
					</td>
				</c:forEach>
			</tr>
		</table>
